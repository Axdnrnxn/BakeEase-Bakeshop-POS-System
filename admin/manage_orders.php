...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_order_status'])) {
    ...
    header("Location: manage_orders.php?success=" . urlencode($updateMessage));
    exit();
}
...
<body class="bg-gray-100 text-gray-800 font-bembo min-h-screen flex flex-col">
    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="flex justify-center gap-4">
                <button id="show-all-btn" class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 transition-colors">
                    All Orders
                </button>
                <button id="pickup-btn" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors">
                    Pickup Orders
                </button>
                <button id="delivery-btn" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors">
                    Delivery Orders
                </button>
            </div>
        </div>
        ...
        <section class="manage-orders bg-white p-8 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex items-center">
                    ...
                    <form method="get" action="manage_orders.php" class="flex items-center">
                        <select name="sort_by" class="border rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary">
                            <option value="date">Date</option>
                            <option value="status">Status</option>
                            <option value="name">Customer Name</option>
                        </select>
                        <button type="submit" class="bg-green-500 text-white font-bold py-1 px-2 rounded ml-2 hover:bg-green-600 transition-colors">
                            Sort
                        </button>
                    </form>
                </div>
                            ...
                            <div class="order-card bg-white rounded-lg shadow-md overflow-hidden border border-gray-200"
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex justify-between items-center mt-4">
                                        <form method='post' action='manage_orders.php' class="inline-block flex-1 mr-2">
                                            <input type='hidden' name='order_id' value='<?= $order['id'] ?>'>
                                            <div class="flex">
                                                <select name='new_status' class="border rounded-l px-1 py-1 focus:outline-none focus:ring-2 focus:ring-primary flex-1 w-30"> <!-- Adjusted padding and width -->
                                                ...
                                                <button type='submit' name='update_order_status'
                                                        class="bg-green-500 text-white font-bold py-1 px-2 rounded-r hover:bg-green-600 transition-colors">
                                                    Update
                                                </button>
                                            </div>
                                        </form>
                                        <a href='../../actions/admin-order-actions.php?archive_order=<?= $order['id'] ?>'
    ...
    <script>
        ...
        function setActiveBtn(activeBtn, inactiveBtn1, inactiveBtn2) {
            ...
            inactiveBtn1.classList.remove('bg-green-500', 'text-white');
            inactiveBtn1.classList.add('bg-gray-200', 'text-gray-800');

            inactiveBtn2.classList.remove('bg-green-500', 'text-white');
            inactiveBtn2.classList.add('bg-gray-200', 'text-gray-800');
        }
